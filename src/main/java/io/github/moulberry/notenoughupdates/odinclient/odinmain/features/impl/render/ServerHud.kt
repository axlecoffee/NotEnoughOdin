...
object ServerHud : Module(
    ...
    val hud by HudSetting("Display", 10f, 10f, 1f, false) {
        if (it) {
            ...
            if (style == 0) {
                val fpsText = "§rFPS: §f${Minecraft.getMinecraft().debug.split(" ")[0].toIntOrNull() ?: 0}"
                ...
                if (fps) RenderUtils.drawText("§dFPS: §a${Minecraft.getMinecraft().debug.split(" ")[0].toIntOrNull() ?: 0}", 1f, 43f, 2f, Colors.WHITE, shadow = true, center = false)
                max(
                    ...
                    if (fps) getTextWidth("§dFPS: ${colorizeFPS(Minecraft.getMinecraft().debug.split(" ")[0].toIntOrNull() ?: 0)}", 12f) else 0f
                ) + 2f to if (ping && tps && fps) 50f else if (ping && tps || ping && fps || tps && fps) 35f else 20f
            }
        }
    }
