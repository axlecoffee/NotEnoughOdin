...
import io.github.moulberry.notenoughupdates.odinclient.odinmain.features.impl.dungeon.dungeonwaypoints.SecretWaypoints.onEtherwarp
import io.github.moulberry.notenoughupdates.odinclient.odinmain.features.impl.dungeon.dungeonwaypoints.SecretWaypoints.onLocked
import io.github.moulberry.notenoughupdates.odinclient.odinmain.features.impl.dungeon.dungeonwaypoints.SecretWaypoints.onPosUpdate
...
import io.github.moulberry.notenoughupdates.odinclient.odinmain.utils.render.RenderUtils.outlineBounds
...
object DungeonWaypoints : Module(
    ...
    private inline val reachPosition: BlockPos? get() =
        Minecraft.getMinecraft()?.takeUnless {
            it.objectMouseOver?.typeOfHit == MovingObjectType.MISS || (distance <= 4.5 && allowMidair)
        }?.objectMouseOver?.blockPos ?: EtherWarpHelper.getEtherPos(
            Minecraft.getMinecraft().thePlayer.renderVec,
    ...
    fun onRenderOverlay(event: RenderGameOverlayEvent.Post) {
        if (Minecraft.getMinecraft().currentScreen != null || event.type != RenderGameOverlayEvent.ElementType.ALL || !allowEdits) return
        ...
        RenderUtils.drawText(editText, Minecraft.getMinecraft().displayWidth / 4f, Minecraft.getMinecraft().displayHeight  / 4f + 10, 1f, Colors.WHITE.withAlpha(.8f), center = true)
        RenderUtils.drawText(text, Minecraft.getMinecraft().displayWidth / 4f,  Minecraft.getMinecraft().displayHeight / 4f + 20, 1f, selectedColor, center = true)
    ...
    fun onInteract(event: ClickEvent.Right) {
        if (Minecraft.getMinecraft().thePlayer.usingEtherWarp) {
            val pos = EtherWarpHelper.getEtherPos(Minecraft.getMinecraft().thePlayer.renderVec, Minecraft.getMinecraft().thePlayer.rotationYaw, Minecraft.getMinecraft().thePlayer.rotationPitch)
        ...
        val aabb =
            if (useBlockSize && block !is BlockSign) block.getSelectedBoundingBox(Minecraft.getMinecraft().theWorld, BlockPos(0, 0, 0))?.outlineBounds() ?: return
        ...
        if (allowTextEdit && Minecraft.getMinecraft().thePlayer?.isSneaking == true) {
