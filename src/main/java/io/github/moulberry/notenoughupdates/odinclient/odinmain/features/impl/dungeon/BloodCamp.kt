...
import io.github.moulberry.notenoughupdates.odinclient.odinmain.events.impl.EntityLeaveWorldEvent
...
import io.github.moulberry.notenoughupdates.odinclient.odinmain.utils.ServerUtils.averagePing
...
import io.github.moulberry.notenoughupdates.odinclient.odinmain.utils.render.mcText
...
object BloodCamp : Module(
    ...
    init {
        onPacket<S17PacketEntityLookMove> ({ bloodAssist && enabled }) { packet ->
            ...
            val world = Minecraft.getMinecraft().theWorld ?: return@onPacket
    ...
    fun onPostMetadata(event: PostEntityMetadata) {
        ...
        currentWatcherEntity = (Minecraft.getMinecraft().theWorld?.getEntityByID(event.packet.entityId) as? EntityZombie)?.takeIf { getSkullValue(it) in watcherSkulls } ?: return
