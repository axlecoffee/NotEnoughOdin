...
public class CrystalWishingCompassSolver {
	...
	public void onPlayerInteract(PlayerInteractEvent event) {
		...
		BlockPos playerPos = Minecraft.getMinecraft().thePlayer.getPosition().getImmutable();
	...
	private void showSolution() {
		...
		if (NotEnoughUpdates.INSTANCE.config.mining.wishingCompassAutocreateKnownWaypoints &&
			...
			Utils.addChatMessage(destinationMessage);
			int commandResult = ClientCommandHandler.instance.executeCommand(Minecraft.getMinecraft().thePlayer, skytilsCommand);
			if (commandResult == 1) {
				return;
			}
			Utils.addChatMessage(
	...
	private String getDiagnosticMessage() {
		...
		diagsMessage.append(EnumChatFormatting.WHITE);
		diagsMessage.append(calculatePossibleTargets(Minecraft.getMinecraft().thePlayer.getPosition()));
		diagsMessage.append("\n");
