...
import io.github.moulberry.notenoughupdates.odinclient.odinmain.utils.render.RenderUtils.tessellator
import io.github.moulberry.notenoughupdates.odinclient.odinmain.utils.render.RenderUtils.worldRenderer
...
import io.github.moulberry.notenoughupdates.odinclient.odinmain.utils.skyblock.getSkullValue
import io.github.moulberry.notenoughupdates.odinclient.odinmain.utils.skyblock.skullTexture
...
object RenderOptimizer : Module(
    ...
    fun onPacket(event: PacketEvent.Receive) {
        ...
        if (event.packet is S1CPacketEntityMetadata && hide0HealthNames) {
            Minecraft.getMinecraft().theWorld?.getEntityByID(event.packet.entityId)?.let { entity ->
    ...
    fun renderEntities(event: RenderWorldLastEvent) {
        Minecraft.getMinecraft().mcProfiler.endStartSection("entities2")
        ...
        GlStateManager.translate(-Minecraft.getMinecraft().renderManager.viewerPosX, -Minecraft.getMinecraft().renderManager.viewerPosY, -Minecraft.getMinecraft().renderManager.viewerPosZ)
    ...
    fun hookRenderEntities(renderViewEntity: Entity, camera: ICamera, partialTicks: Float, ci: CallbackInfo) {
        ...
        Minecraft.getMinecraft().renderManager.cacheActiveRenderInfo(Minecraft.getMinecraft().theWorld, Minecraft.getMinecraft().fontRendererObj, Minecraft.getMinecraft().renderViewEntity, Minecraft.getMinecraft().pointedEntity, Minecraft.getMinecraft().gameSettings, partialTicks)
        ...
        Minecraft.getMinecraft().renderManager.setRenderPosition(viewX, viewY, viewZ)

        renderTileEntities(camera, partialTicks)

        for (entity in Minecraft.getMinecraft().theWorld.loadedEntityList) {
            if (entity.isInvisible || (Minecraft.getMinecraft().gameSettings.thirdPersonView == 0 && entity == Minecraft.getMinecraft().renderViewEntity) ||
                (entity.posY < 0.0 || entity.posY >= 256.0 || !Minecraft.getMinecraft().theWorld.isBlockLoaded(BlockPos(entity)))) continue
            if (!(Minecraft.getMinecraft().renderManager.shouldRender(entity, camera, Minecraft.getMinecraft().renderManager.viewerPosX, Minecraft.getMinecraft().renderManager.viewerPosY, Minecraft.getMinecraft().renderManager.viewerPosZ))) continue
            ...
            if (showStarredNametags && entity is EntityArmorStand && entity.name.startsWith("§6✯ ")) {
                Minecraft.getMinecraft().renderManager.renderEntitySimple(entity, partialTicks)
                continue
            }
        ...
        for (entity in defaultRenderList) { Minecraft.getMinecraft().renderManager.renderEntitySimple(entity, partialTicks) }
        defaultRenderList.clear()
    }

    private fun renderTileEntities(camera: ICamera, partialTicks: Float) {
        TileEntityRendererDispatcher.instance.cacheActiveRenderInfo(Minecraft.getMinecraft().theWorld, Minecraft.getMinecraft().textureManager, Minecraft.getMinecraft().fontRendererObj, Minecraft.getMinecraft().renderViewEntity, partialTicks)
        ...
        for (tileEntity in Minecraft.getMinecraft().theWorld.loadedTileEntityList) {
