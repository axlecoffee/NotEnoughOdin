...
object TerminalSounds : Module(
    ...
    fun onTermComplete(event: TerminalEvent.Solved) {
        if (shouldReplaceSounds && (!completeSounds && !clickSounds)) Minecraft.getMinecraft().thePlayer.playSound("note.pling", 8f, 4f)
    ...
    init {
        onMessage(Regex("The gate has been destroyed!"), { enabled && shouldReplaceSounds }) { Minecraft.getMinecraft().thePlayer.playSound("note.pling", 8f, 4f) }

        onMessage(Regex("The Core entrance is opening!"), { enabled && shouldReplaceSounds }) { Minecraft.getMinecraft().thePlayer.playSound("note.pling", 8f, 4f) }
    }
