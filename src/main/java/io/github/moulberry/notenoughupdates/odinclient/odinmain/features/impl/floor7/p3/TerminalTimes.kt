...
object TerminalTimes : Module(
    ...
    fun onTerminalSolved(event: TerminalEvent.Solved) {
        val pbs = if (Minecraft.getMinecraft().currentScreen is TermSimGUI) TerminalSimulator.termSimPBs else terminalPBs
        pbs.time(event.terminal.type.ordinal, (System.currentTimeMillis() - event.terminal.timeOpened) / 1000.0, "s§7!", "§a${event.terminal.type.windowName}${if (Minecraft.getMinecraft().currentScreen is TermSimGUI) " §7(termsim)" else ""} §7solved in §6", addPBString = true, addOldPBString = true, sendOnlyPB = sendMessage)
    }
