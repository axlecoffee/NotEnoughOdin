...
object ChocolateFactory : Module(
    ...
    init {
        ...
        execute(delay = { delay }) {
            ...
            val found = (Minecraft.getMinecraft().thePlayer?.openContainer as? ContainerChest)?.inventorySlots?.find { it.stack?.displayName?.containsOneOf("CLICK ME!", "Golden Rabbit") == true } ?: return@execute
        ...
        onMessage(Regex(".*(A|found|collected).+Chocolate (Breakfast|Lunch|Dinner|Brunch|Déjeuner|Supper).*")) {
            if (it.groupValues.getOrNull(1).equalsOneOf("found", "collected"))
                currentDetectedEggs.minByOrNull { egg -> egg.entity.getDistanceToEntity(Minecraft.getMinecraft().thePlayer) }?.isFound = true
        }
    }
    ...
    private fun isInChocolateFactory(): Boolean {
        return Minecraft.getMinecraft().thePlayer?.openContainer is ContainerChest && Minecraft.getMinecraft().thePlayer?.openContainer?.name == "Chocolate Factory"
    }

    private object BunnyEggTextures {
