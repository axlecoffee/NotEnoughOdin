...
import io.github.moulberry.notenoughupdates.odinclient.odinmain.events.impl.MessageSentEvent
...
object ChatCommands : Module(
    ...
    init {
        onMessage(Regex(" {29}> EXTRA STATS <|^\\[NPC] Elle: Good job everyone. A hard fought battle come to an end. Let's get out of here before we run into any more trouble!$")) {
            ...
            runIn(30) {
                dtReason.find { it.first == Minecraft.getMinecraft().thePlayer.name }?.let { partyMessage("Downtime needed: ${it.second}") }
    ...
    private fun handleChatCommands(message: String, name: String, channel: ChatChannel) {
        ...
        when (words[0]) {
            ...
            "fps" -> if (fps) channelMessage("Current FPS: ${Minecraft.getMinecraft().debug.split(" ")[0].toIntOrNull() ?: 0}", name, channel)
            ...
            "holding" -> if (holding) channelMessage("Holding: ${Minecraft.getMinecraft().thePlayer?.heldItem?.displayName?.noControlCodes ?: "Nothing :("}", name, channel)
