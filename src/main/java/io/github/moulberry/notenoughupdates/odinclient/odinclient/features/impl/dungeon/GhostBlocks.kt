...
import io.github.moulberry.notenoughupdates.odinclient.odinclient.utils.skyblock.PlayerUtils.swapToIndex
...
import io.github.moulberry.notenoughupdates.odinclient.odinmain.utils.addRange
...
import io.github.moulberry.notenoughupdates.odinclient.odinmain.utils.skyblock.dungeon.DungeonUtils.getF7Phase
...
import io.github.moulberry.notenoughupdates.odinclient.odinmain.utils.skyblock.getItemSlot
...
object GhostBlocks : Module(
    ...
    private fun giveItem(id: Int) {
        if (!enabled || Minecraft.getMinecraft().thePlayer == null || Minecraft.getMinecraft().currentScreen != null) return
        runIn(delay / 50) {
            ...
            Minecraft.getMinecraft().thePlayer?.inventory?.mainInventory?.set(slot - 1, item)
        }
    }
    ...
    private val swapStonkKey by KeybindSetting("Swap Stonk Keybind", Keyboard.KEY_NONE, "Press to perform a swap stonk.").withDependency { swapStonkDropDown }
        .onPress {
            ...
            val originalItem = Minecraft.getMinecraft().thePlayer?.inventory?.currentItem ?: 0
    ...
    init {
        execute({ ghostBlockSpeed }) {
            if (!enabled || Minecraft.getMinecraft().currentScreen != null || (onlyDungeon && !inDungeons)) return@execute
            if (!ghostBlockKey.isDown()) return@execute

            toAir(Minecraft.getMinecraft().thePlayer?.rayTrace(ghostBlockRange, 1f)?.blockPos ?: return@execute)
        }
        ...
        execute(1000) {
            ...
            for (i in blocks[phase] ?: return@execute) {
                Minecraft.getMinecraft().theWorld?.setBlockToAir(i)
            }
            for (i in enderChests[phase] ?: return@execute) {
                Minecraft.getMinecraft().theWorld?.setBlockState(i, Blocks.ender_chest.defaultState)
            }
            for (i in glass[phase] ?: return@execute) {
                Minecraft.getMinecraft().theWorld?.setBlockState(i, Blocks.stained_glass.defaultState)
            }
        }
    ...
    private fun toAir(blockPos: BlockPos) {
        getBlockAt(blockPos).let { block ->
            if (block !in blacklist && (block !== Blocks.skull || (ghostBlockSkulls && (Minecraft.getMinecraft().theWorld?.getTileEntity(blockPos) as? TileEntitySkull)
                    ?.playerProfile?.id?.toString() != WITHER_ESSENCE_ID))) Minecraft.getMinecraft().theWorld?.setBlockToAir(blockPos)
        }
    }
    ...
    private fun BlockData.reset() = Minecraft.getMinecraft().theWorld?.setBlockState(pos, state)

    @JvmStatic
    fun breakBlock(pos: BlockPos) {
        if (!stonkDelayToggle || (sdOnlySB && !LocationUtils.isInSkyblock)) return
        sdBlocks.add(BlockData(pos, Minecraft.getMinecraft().theWorld.getBlockState(pos), System.currentTimeMillis(), false))
    }
    ...
    fun postChunkData(packet: S21PacketChunkData) {
        ...
        sdBlocks.forEach {
            if (it.pos.x in (packet.chunkX shl 4).addRange(15) && it.pos.z in (packet.chunkZ shl 4).addRange(15)) {
                Minecraft.getMinecraft().theWorld?.setBlockState(it.pos, Blocks.air.defaultState)
                it.serverReplaced = true
                it.state = Minecraft.getMinecraft().theWorld.getBlockState(it.pos)
            }
        }
    }
