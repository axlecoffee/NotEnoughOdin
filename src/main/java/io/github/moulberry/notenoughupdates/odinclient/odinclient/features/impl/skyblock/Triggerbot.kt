...
import io.github.moulberry.notenoughupdates.odinclient.odinmain.features.impl.floor7.p3.ArrowAlign.clicksRemaining
import io.github.moulberry.notenoughupdates.odinclient.odinmain.features.impl.floor7.p3.ArrowAlign.currentFrameRotations
...
object Triggerbot : Module(
    ...
    fun onEntityJoin(event: EntityJoinWorldEvent) {
        if (event.entity !is EntityOtherPlayerMP || Minecraft.getMinecraft().currentScreen != null || !DungeonUtils.inDungeons) return
    ...
    fun onClientTickEvent(event: TickEvent.ClientTickEvent) {
        if (crystal && DungeonUtils.inBoss && DungeonUtils.getF7Phase() == M7Phases.P1 && clickClock.hasTimePassed() && Minecraft.getMinecraft().objectMouseOver != null) {
            if ((take && Minecraft.getMinecraft().objectMouseOver.entityHit is EntityEnderCrystal) || (place && Minecraft.getMinecraft().objectMouseOver.entityHit?.name?.noControlCodes == "Energy Crystal Missing" && Minecraft.getMinecraft().thePlayer?.heldItem?.unformattedName == "Energy Crystal")) {
        ...
        if (relicTriggerBot && tbClock.hasTimePassed()) {
            val obj = Minecraft.getMinecraft().objectMouseOver ?: return
            when {
                obj.entityHit is EntityArmorStand && obj.entityHit?.inventory?.get(4)?.skyblockID in cauldronMap.keys -> {
                    PlayerUtils.rightClick()
                    tbClock.update()
                }
                Vec2(obj.blockPos?.x ?: 0, obj.blockPos?.z ?: 0) == cauldronMap[Minecraft.getMinecraft().thePlayer?.heldItem?.skyblockID ?: ""] && obj.blockPos?.y.equalsOneOf(6, 7) -> {
                    PlayerUtils.rightClick()
                    tbClock.update()
                }
            }
    ...
    init {
        ...
        execute(0) {
            ...
            val pos = Minecraft.getMinecraft().objectMouseOver?.blockPos ?: return@execute
            val state = Minecraft.getMinecraft().theWorld?.getBlockState(pos) ?: return@execute
    ...
    private fun arrowAlignTriggerbot() {
        if ((sneakToDisableTriggerbot && Minecraft.getMinecraft().thePlayer.isSneaking) || clicksRemaining.isEmpty()) return
        val targetFrame = Minecraft.getMinecraft().objectMouseOver?.entityHit as? EntityItemFrame ?: return
