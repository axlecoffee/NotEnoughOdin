...
object ArrowAlign : Module(
    ...
    init {
        execute(50) {
            ...
            if ((Minecraft.getMinecraft().thePlayer?.positionVector?.distanceTo(Vec3(0.0, 120.0, 77.0)) ?: return@execute) > 200) {
    ...
    fun onPacket(event: PacketEvent.Send) {
        ...
        val (x, y, z) = (packet.getEntityFromWorld(Minecraft.getMinecraft().theWorld) as? EntityItemFrame)?.takeIf { it.displayedItem?.item == Items.arrow }?.positionVector?.floorVec() ?: return
        ...
        if (!clicksRemaining.containsKey(frameIndex) && Minecraft.getMinecraft().thePlayer.isSneaking == invertSneak && blockWrong) {
    ...
    private fun getFrames(): List<Int> {
        val itemFrames = Minecraft.getMinecraft().theWorld?.loadedEntityList?.mapNotNull {
