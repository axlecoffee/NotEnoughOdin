...
import io.github.moulberry.notenoughupdates.odinclient.odinmain.features.impl.floor7.p3.termGUI.CustomTermGui
import io.github.moulberry.notenoughupdates.odinclient.odinmain.features.impl.floor7.p3.terminalhandler.*
...
object TerminalSolver : Module(
    ...
    init {
        ...
        execute(50) {
            if (System.currentTimeMillis() - lastClickTime >= terminalReloadThreshold && currentTerm?.isClicked == true) currentTerm?.let {
                PacketEvent.Receive(S2FPacketSetSlot(Minecraft.getMinecraft().thePlayer?.openContainer?.windowId ?: return@execute, it.type.windowSize - 1, null)).postAndCatch()
                it.isClicked = false
                isReload = true
            }
        }
        ...
        onMessage(Regex("(.{1,16}) activated a terminal! \\((\\d)/(\\d)\\)")) { message ->
            if (message.groupValues[1] == Minecraft.getMinecraft().thePlayer.name) lastTermOpened?.let { TerminalEvent.Solved(it).postAndCatch() }
        }
    }
    ...
    fun onGuiKeyPress(event: GuiEvent.KeyPress) {
        ...
        if ((event.key == Minecraft.getMinecraft().gameSettings?.keyBindDrop?.keyCode || (event.key in 2..10))) {
            CustomTermGui.mouseClicked(MouseUtils.mouseX.toInt(), MouseUtils.mouseY.toInt(), if (event.key == Keyboard.KEY_LCONTROL && event.key == Minecraft.getMinecraft().gameSettings.keyBindDrop.keyCode) 1 else 0)
            event.isCanceled = true
        }
    }
