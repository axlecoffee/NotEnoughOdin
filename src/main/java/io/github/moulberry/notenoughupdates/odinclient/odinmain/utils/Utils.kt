...
fun startProfile(name: String) =
    Minecraft.getMinecraft().mcProfiler.startSection("Odin: $name")
...
fun fillItemFromSack(amount: Int, itemId: String, sackName: String, sendMessage: Boolean) {
    val needed = Minecraft.getMinecraft().thePlayer?.inventory?.mainInventory?.find { it?.skyblockID == itemId }?.stackSize ?: 0
...
fun runOnMCThread(run: () -> Unit) {
    if (!Minecraft.getMinecraft().isCallingFromMinecraftThread) Minecraft.getMinecraft().addScheduledTask(run) else run()
}

fun EntityPlayer?.isOtherPlayer(): Boolean {
    return this != null && this != Minecraft.getMinecraft().thePlayer && this.uniqueID.version() != 2
}
