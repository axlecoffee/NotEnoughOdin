...
fun fastEyeHeight(): Float =
    if (Minecraft.getMinecraft().thePlayer?.isSneaking == true) 1.54f else 1.62f
 ...
 */
fun getPositionEyes(pos: Vec3 = Minecraft.getMinecraft().thePlayer?.positionVector ?: Vec3(0.0, 0.0, 0.0)): Vec3 = Vec3(pos.xCoord, pos.yCoord + fastEyeHeight(), pos.zCoord)
 ...
 */
fun getLook(yaw: Float = Minecraft.getMinecraft().thePlayer?.rotationYaw ?: 0f, pitch: Float = Minecraft.getMinecraft().thePlayer?.rotationPitch ?: 0f): Vec3 {
 ...
 */
fun isFacingAABB(aabb: AxisAlignedBB, range: Float, yaw: Float = Minecraft.getMinecraft().thePlayer?.rotationYaw ?: 0f, pitch: Float = Minecraft.getMinecraft().thePlayer?.rotationPitch ?: 0f): Boolean =
 ...
 * @see io.github.moulberry.notenoughupdates.odinclient.odinmain.utils.skyblock.DianaBurrowEstimate.guessPosition
 * @author Soopy
 */
...
fun findNearestGrassBlock(pos: Vec3): Vec3 {
    val chunk = Minecraft.getMinecraft().theWorld?.getChunkFromBlockCoords(BlockPos(pos)) ?: return pos.coerceYIn(50.0, 110.0)
...
fun etherwarpRotateTo(targetPos: BlockPos, dist: Double = 61.0): Triple<Double, Float, Float>? {
    val distance = Minecraft.getMinecraft().thePlayer?.getDistanceSq(targetPos) ?: return null
    ...
    for (targetVec in targets) {
        ...
        val obj = Minecraft.getMinecraft().theWorld?.rayTraceBlocks(eyeVec, vec32, true, false, true) ?: return null
...
fun smoothRotateTo(yaw: Float, pitch: Float, rotTime: Number, functionToRunWhenDone: () -> Unit = {}) {
    scope.launch {
        val initialYaw = MathHelper.wrapAngleTo180_float(Minecraft.getMinecraft().thePlayer.rotationYaw)
        val initialPitch = MathHelper.wrapAngleTo180_float(Minecraft.getMinecraft().thePlayer.rotationPitch)
        ...
        for (event in tickerChannel) {
            ...
            Minecraft.getMinecraft().thePlayer?.rotationYaw = initialYaw + (targetYaw - initialYaw) * amount
            Minecraft.getMinecraft().thePlayer?.rotationPitch = initialPitch + (targetPitch - initialPitch) * amount
