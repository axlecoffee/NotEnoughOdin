...
package io.github.moulberry.notenoughupdates.odinclient.odinmain.utils.skyblock.dungeon
...
import io.github.moulberry.notenoughupdates.odinclient.odinmain.features.impl.dungeon.MapInfo.togglePaul
...
import io.github.moulberry.notenoughupdates.odinclient.odinmain.utils.skyblock.LocationUtils.currentDungeon
...
object DungeonUtils {
    ...
    inline val currentDungeonPlayer: DungeonPlayer
        get() = dungeonTeammates.find { it.name == Minecraft.getMinecraft().thePlayer?.name } ?: DungeonPlayer(Minecraft.getMinecraft().thePlayer?.name ?: "Unknown", DungeonClass.Unknown, 0, entity = Minecraft.getMinecraft().thePlayer)
    ...
    fun getDungeonTeammates(previousTeammates: ArrayList<DungeonPlayer>, tabList: List<String>): ArrayList<DungeonPlayer> {
        for (line in tabList) {
            ...
            previousTeammates.add(DungeonPlayer(name, DungeonClass.entries.find { it.name == clazz } ?: continue, clazzLvl = romanToInt(clazzLevel), Minecraft.getMinecraft().netHandler?.getPlayerInfo(name)?.locationSkin ?: continue, Minecraft.getMinecraft().theWorld?.getPlayerEntityByName(name), false))
        }
    ...
    fun isSecret(state: IBlockState, pos: BlockPos): Boolean {
        return when {
            ...
            state.block is BlockSkull -> (Minecraft.getMinecraft().theWorld?.getTileEntity(pos) as? TileEntitySkull)?.playerProfile?.id.toString().equalsOneOf(WITHER_ESSENCE_ID, REDSTONE_KEY)
            else -> false
        }
    }
