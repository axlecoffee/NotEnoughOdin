...
import io.github.moulberry.notenoughupdates.odinclient.odinmain.OdinMain.VERSION
...
import io.github.moulberry.notenoughupdates.odinclient.odinmain.features.ModuleManager.generateFeatureList
import io.github.moulberry.notenoughupdates.odinclient.odinmain.features.impl.dungeon.MapInfo
import io.github.moulberry.notenoughupdates.odinclient.odinmain.features.impl.floor7.DragonPriority.displaySpawningDragon
import io.github.moulberry.notenoughupdates.odinclient.odinmain.features.impl.floor7.DragonPriority.findPriority
import io.github.moulberry.notenoughupdates.odinclient.odinmain.features.impl.floor7.WitherDragonState
import io.github.moulberry.notenoughupdates.odinclient.odinmain.features.impl.floor7.WitherDragons.priorityDragon
import io.github.moulberry.notenoughupdates.odinclient.odinmain.features.impl.floor7.WitherDragonsEnum
import io.github.moulberry.notenoughupdates.odinclient.odinmain.features.impl.nether.NoPre
import io.github.moulberry.notenoughupdates.odinclient.odinmain.features.impl.render.RandomPlayers.updateCustomProperties
...
import io.github.moulberry.notenoughupdates.odinclient.odinmain.utils.skyblock.dungeon.ScanUtils.getRoomCenter
...
val devCommand = Commodore("oddev") {
    ...
    literal("customSize").runs { password: String, xSize: Float?, ySize: Float?, zSize: Float?, customName: String? ->
        ...
        val name = customName ?: ""
        scope.launch {
            modMessage(sendDataToServer(body = "${Minecraft.getMinecraft().thePlayer.name}, [1,2,3], [$x,$y,$z], false, $name, $password", "https://tj4yzotqjuanubvfcrfo7h5qlq0opcyk.lambda-url.eu-north-1.on.aws/"))
            updateCustomProperties()
        }
    }
    ...
    literal("simulate").runs { str: GreedyString ->
        Minecraft.getMinecraft().thePlayer.addChatMessage(ChatComponentText(str.string))
    ...
    literal("relativecoords").runs {
        val block = Minecraft.getMinecraft().objectMouseOver?.blockPos ?: return@runs
        modMessage(
            """
            ${getChatBreak()}
            Middle: $block
    ...
    literal("getplayers").runs {
        val playerEntities = Minecraft.getMinecraft().theWorld?.playerEntities?.filter { it.isOtherPlayer() } ?: return@runs modMessage("No players found.")
