...
import io.github.moulberry.notenoughupdates.odinclient.odinmain.utils.skyblock.drawItem
...
object DeployableTimer : Module(
    ...
    private val hud by HudSetting("Display", 10f, 10f, 1f, false) {
        if (it) {
            ...
            val activeDeployable = activeDeployables.firstOrNull { dep -> Minecraft.getMinecraft().thePlayer.getDistanceToEntity(dep.entity) <= dep.deployable.range } ?: return@HudSetting 0f to 0f
    ...
    fun postMetadata(event: PostEntityMetadata) {
        var entity = Minecraft.getMinecraft().theWorld?.getEntityByID(event.packet.entityId) as? EntityArmorStand ?: return
        ...
        val duration =
            if (deployable.texture == "placeholder") {
                entity = Minecraft.getMinecraft().theWorld?.getEntitiesWithinAABBExcludingEntity(entity, entity.entityBoundingBox.offset(0.0, -2.0, 0.0).expand(0.1, 2.0, 0.1))?.filterIsInstance<EntityArmorStand>()?.firstOrNull() ?: return
