...
object ArrowsDevice : Module(
    ...
    init {
        ...
        onMessage(Regex("^(.{1,16}) completed a device! \\((\\d)/(\\d)\\)")) {
            if (it.groupValues[1] == Minecraft.getMinecraft().thePlayer.name) onComplete("Device Complete Message")
        }
        ...
        execute(1000) {
            ...
            activeArmorStand = Minecraft.getMinecraft().theWorld?.loadedEntityList?.filterIsInstance<EntityArmorStand>()?.find {
        ...
        execute(10) {
            if (!triggerBot || !triggerBotClock.hasTimePassed(triggerBotDelay) || Minecraft.getMinecraft().thePlayer?.heldItem?.isShortbow == false || DungeonUtils.getF7Phase() != M7Phases.P3) return@execute
    ...
    private fun setBowTrajectoryHeading(yawOffset: Float) {
        val yawRadians = ((Minecraft.getMinecraft().thePlayer.rotationYaw + yawOffset) / 180) * Math.PI.toFloat()
        val pitchRadians = (Minecraft.getMinecraft().thePlayer.rotationPitch / 180) * Math.PI.toFloat()

        var posX = Minecraft.getMinecraft().thePlayer.renderX
        var posY = Minecraft.getMinecraft().thePlayer.renderY + Minecraft.getMinecraft().thePlayer.eyeHeight
        var posZ = Minecraft.getMinecraft().thePlayer.renderZ
        posX -= (MathHelper.cos(Minecraft.getMinecraft().thePlayer.rotationYaw / 180.0f * Math.PI.toFloat()) * 0.16f).toDouble()
        posY -= 0.1
        posZ -= (MathHelper.sin(Minecraft.getMinecraft().thePlayer.rotationYaw / 180.0f * Math.PI.toFloat()) * 0.16f).toDouble()
    ...
    private fun calculateBowTrajectory(mV: Vec3, pV: Vec3) {
        ...
        repeat(21) {
            ...
            val rayTrace = Minecraft.getMinecraft().theWorld?.rayTraceBlocks(posVec, vec, false, true, false)
    ...
    private val isDeviceRoomOpen: Boolean
        get() = Minecraft.getMinecraft().theWorld?.getBlockState(lastGateBlock) == Blocks.air.defaultState

    private val isPlayerOnStand: Boolean
        get() = (Minecraft.getMinecraft().thePlayer?.positionVector?.distanceTo(standPosition.toVec3()) ?: Double.MAX_VALUE) <= 1.0

    private val isPlayerInRoom: Boolean
        get() = Minecraft.getMinecraft().thePlayer?.let { roomBoundingBox.isVecInside(it.positionVector) } == true
    ...
    private fun holdBow() {
        if ((Minecraft.getMinecraft().thePlayer?.inventory?.currentItem ?: -1) != bowSlot)
            Minecraft.getMinecraft().thePlayer.inventory.currentItem = bowSlot
    }

    private fun phoenixSwap() {
        val rodSlot = Minecraft.getMinecraft().thePlayer?.inventory?.mainInventory?.indexOfFirst { it.isFishingRod } ?: -1
    ...
    private fun leap() {
        val leapSlot = Minecraft.getMinecraft().thePlayer?.inventory?.mainInventory?.indexOfFirst { it.isLeap } ?: -1
    ...
    fun onBlockChange(event: BlockChangeEvent) {
        ...
        if (event.old.block == Blocks.stained_hardened_clay && event.updated.block == Blocks.emerald_block) {
            ...
            if (isPlayerOnStand && auto) {
                if (autoState == AutoState.Stopped) {
                    bowSlot = Minecraft.getMinecraft().thePlayer?.inventory?.mainInventory?.indexOfFirst { it.isShortbow } ?: -1
