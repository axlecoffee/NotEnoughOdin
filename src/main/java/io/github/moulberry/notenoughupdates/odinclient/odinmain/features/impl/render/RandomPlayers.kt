...
import io.github.moulberry.notenoughupdates.odinclient.odinmain.features.impl.render.ClickGUIModule.devSize
import io.github.moulberry.notenoughupdates.odinclient.odinmain.utils.downloadFile
import io.github.moulberry.notenoughupdates.odinclient.odinmain.utils.getDataFromServer
...
object RandomPlayers {
    ...
    val isRandom get() = randoms.containsKey(Minecraft.getMinecraft().session?.username)
    val isDev get() = randoms[Minecraft.getMinecraft().session?.username]?.isDev ?: false
    ...
    fun preRenderCallbackScaleHook(entityLivingBaseIn: AbstractClientPlayer) {
        ...
        if (!devSize && entityLivingBaseIn.name == Minecraft.getMinecraft().thePlayer.name) return
    ...
    fun onRenderPlayer(event: RenderPlayerEvent.Post) {
        ...
        if (!devSize && event.entity.name == Minecraft.getMinecraft().thePlayer.name) return
    ...
    object DragonWings : ModelBase() {
        ...
        fun renderWings(player: EntityPlayer, partialTicks: Float, random: RandomPlayer) {
            ...
            GlStateManager.translate(-Minecraft.getMinecraft().renderManager.viewerPosX + x, -Minecraft.getMinecraft().renderManager.viewerPosY + y, -Minecraft.getMinecraft().renderManager.viewerPosZ + z)
            ...
            Minecraft.getMinecraft().textureManager.bindTexture(dragonWingTextureLocation)
    ...
    private val capeFolder = File(Minecraft.getMinecraft().mcDataDir, "config/odin/capes")
    ...
    private fun getCapeLocation(dev: RandomPlayer?, file: File): ResourceLocation? {
        if (dev?.capeLocation == null && file.exists()) {
            try {
                ...
                val capeLocation = Minecraft.getMinecraft().textureManager.getDynamicTextureLocation("odincapes", DynamicTexture(image))
