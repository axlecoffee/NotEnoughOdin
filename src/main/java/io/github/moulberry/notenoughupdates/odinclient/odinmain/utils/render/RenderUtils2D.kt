...
import io.github.moulberry.notenoughupdates.odinclient.odinmain.utils.render.RenderUtils.bind
...
object RenderUtils2D {
    ...
    fun onRenderWorld(event: RenderWorldLastEvent) {
        val (x, y, z) = Minecraft.getMinecraft().thePlayer?.renderVec ?: return
    ...
    fun drawNameTag(vec3: Vec3, name: String) {
        worldToScreenPosition(vec3)?.let { pos ->
            Minecraft.getMinecraft().fontRendererObj.drawString(name, pos.xCoord.toFloat(), pos.yCoord.toFloat(), -1, true) // can use nvg in the future
        }
    }
