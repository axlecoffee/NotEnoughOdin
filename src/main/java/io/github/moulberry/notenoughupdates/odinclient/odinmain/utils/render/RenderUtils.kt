...
object RenderUtils {
    ...
    private val renderManager: RenderManager = Minecraft.getMinecraft().renderManager
    ...
    fun drawStringInWorld(
        ...
        GlStateManager.rotate(renderManager.playerViewX * if (Minecraft.getMinecraft().gameSettings.thirdPersonView == 2) -1 else 1, 1.0f, 0.0f, 0.0f)
        GlStateManager.scale(-scale, -scale, scale)

        depth(depthTest)

        Minecraft.getMinecraft().fontRendererObj.drawString("$text§r", -Minecraft.getMinecraft().fontRendererObj.getStringWidth(text) / 2f, 0f, color.rgba, shadow)
    ...
    fun drawText(
        ...
        GlStateManager.scale(scale, scale, scale)
        Minecraft.getMinecraft().fontRendererObj.drawString("${text}§r", if (center) Minecraft.getMinecraft().fontRendererObj.getStringWidth(text) / -2f else 0f, 0f, color.rgba, shadow)
        GlStateManager.resetColor()
        GlStateManager.disableBlend()
        GlStateManager.popMatrix()
    }
    ...
    fun drawBeaconBeam(vec3: Vec3, color: Color, depth: Boolean = true, height: Int = 300) {
        ...
        val time: Double = Minecraft.getMinecraft().theWorld.worldTime.toDouble() + partialTicks
        val x = vec3.xCoord
        val y = vec3.yCoord
        val z = vec3.zCoord
    ...
    fun drawMinecraftLabel(str: String, pos: Vec3, scale: Double, depth: Boolean = true, color: Color = Colors.WHITE) {
        ...
        GlStateManager.enableBlend()
        GlStateManager.tryBlendFuncSeparate(770, 771, 1, 0)
        Minecraft.getMinecraft().fontRendererObj.drawString(str, -Minecraft.getMinecraft().fontRendererObj.getStringWidth(str) / 2f, 0f, color.rgba, true)
        GlStateManager.disableBlend()
