...
import io.github.moulberry.notenoughupdates.odinclient.odinmain.utils.fastEyeHeight
import io.github.moulberry.notenoughupdates.odinclient.odinmain.utils.render.RenderUtils.drawBeaconBeam
...
object Renderer {
    ...
    fun drawBlock(
        ...
        drawBox(block.getSelectedBoundingBox(Minecraft.getMinecraft().theWorld, pos).outlineBounds().expand(expand, expand, expand), color, outlineWidth, outlineAlpha, fillAlpha, depth, lineSmoothing)
    }
    ...
    fun drawTracer(goal: Vec3, color: Color, lineWidth: Float = 3f, depth: Boolean = false) {
        RenderUtils.drawLines(listOf(Minecraft.getMinecraft().thePlayer.renderVec.addVec(y = fastEyeHeight()), goal), color, lineWidth, depth)
    }
    ...
    fun drawCustomBeacon(title: String, vec3: Vec3, color: Color, beacon: Boolean = true, increase: Boolean = true, noFade: Boolean = false, distance: Boolean = true, style: Int = 1) {
        val dist = vec3.distanceTo(Minecraft.getMinecraft().thePlayer.positionVector)
