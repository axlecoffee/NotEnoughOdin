...
object SimonSays : Module(
    ...
    private fun start() {
        if (Minecraft.getMinecraft().objectMouseOver?.blockPos == startButton)
            repeat(startClicks) {
    ...
    fun onPostMetadata(event: PostEntityMetadata) {
        if (DungeonUtils.getF7Phase() != M7Phases.P3) return
        val (x, y, z) = (Minecraft.getMinecraft().theWorld?.getEntityByID(event.packet.entityId) as? EntityItem)?.takeIf { Item.getIdFromItem(it.entityItem?.item) == 77 }?.positionVector?.floorVec() ?: return
    ...
    private fun triggerBot() {
        if (!triggerBotClock.hasTimePassed(triggerBotDelay) || clickInOrder.isEmpty() || Minecraft.getMinecraft().currentScreen != null) return
    ...
    private fun autoSS() {
        val isInSSRange = Minecraft.getMinecraft().thePlayer.getDistanceSqToCenter(BlockPos(108, 120, 93)) <= 1.45 * 1.45
    ...
    fun onInteract(event: PlayerInteractEvent) {
        if (
            ...
            event.world != Minecraft.getMinecraft().theWorld
        ) return
        ...
        if (
            blockWrong &&
            Minecraft.getMinecraft().thePlayer?.isSneaking == false &&
