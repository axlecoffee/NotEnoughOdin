...
public class WitherCloakChanger {
	...
	public void onRenderLast(RenderWorldLastEvent event) {
		...
		for (int i = 0; i < NotEnoughUpdates.INSTANCE.config.itemOverlays.customWitherCloakCount; i++) {
			...
			angle %= 360;
			double posX = Minecraft.getMinecraft().thePlayer.posX - (shieldWidth / 2);
			double posY = Minecraft.getMinecraft().thePlayer.posY;
			double posZ = Minecraft.getMinecraft().thePlayer.posZ + NotEnoughUpdates.INSTANCE.config.itemOverlays.customWitherCloakDistance;
			...
			Vec3 bottomLeft = rotateAboutOrigin(Minecraft.getMinecraft().thePlayer.posX, Minecraft.getMinecraft().thePlayer.posZ, angle, new Vec3(posX, posY, posZ));
